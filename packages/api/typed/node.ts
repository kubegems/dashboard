...
interface CordonNode {
  cordonNode(cluster: string, schedulable: boolean): Promise<void>;
  metaNode(cluster: string): Promise<void>;
}
...
export class Node extends V1Node implements CordonNode {
  ...
  // CordonNode
  public async cordonNode(cluster: string, schedulable: boolean): Promise<void> {
    await axios.patch(`proxy/cluster/${cluster}/custom/core/v1/nodes/${this.metadata.name}/actions/cordon`, {
      Unschedulable: !schedulable,
    });
  }

  public async metaNode(cluster: string): Promise<void> {
    await axios.patch(`proxy/cluster/${cluster}/custom/core/v1/nodes/${this.metadata.name}/actions/metadata`, {
