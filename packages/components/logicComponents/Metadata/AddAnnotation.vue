...
<template>
  <BaseDialog
    ...
    icon="mdi-tag-plus"
    :title="i18n.t('operate.add_c', [i18nLocal.t('tip.annotation')])"
    :width="500"
    @reset="reset"
  >
    <template #content>
      <BaseSubTitle :title="i18n.t('form.definition', [i18nLocal.t('tip.annotation')])" />
    ...
    <template #action>
      <v-btn class="float-right" color="primary" :loading="store.state.Circular" text @click="addAnnotation">
...
<script lang="ts" setup>
  ...
  const objRules = {
    key: [required, k8sAnnotation],
    value: [required, k8sAnnotation],
  };
  ...
  const addAnnotation = async (): Promise<void> => {
    if (form.value.validate(true)) {
      ...
      emit('refresh', obj.value, 'annotation');
      reset();
    }
  };
