    ...
    <template #content>
      <v-row class="file__row">
        ...
        <v-col :cols="9" :style="{ position: 'relative' }">
          ...
          <div class="mt-2">
            {{ i18nLocal.t('tip.path') }}:
            <a v-for="(item, index) in currentDirFiles" :key="index" @click.stop="toDir(item)">/{{ item._name }}</a>
            <a v-if="currentDirFiles.length === 0">/</a>
          </div>
          <v-data-table
            class="file__table"
            disable-sort
          ...
          >
            <template #item.name="{ item }">
              ...
              <abbr v-if="item.islink" class="success--text">
                ...
                <a class="success--text" @click.stop="toLinkDir(item)">{{ item.realFile }}</a>
              </abbr>
            </template>
...
<script lang="ts" setup>
  ...
  const currentDirFiles = computed(() => {
    const pathSplit = currentDir.value.split('/').filter((f) => Boolean(f));
    const dirs = [];
    Array.from({ length: pathSplit.length }, (v, i) => i).forEach((index) => {
      const dir = pathSplit
        .map((p, i) => {
          if (i <= index) return p;
        })
      ...
      dirs.push(
        new File({
          _name: pathSplit[index],
          name: dir,
          isDir: true,
          absnameKey: `${dir}`.replaceAll('/', '_').replaceAll('.', '_'),
          absname: `${dir}`.replaceAll('//', '/'),
          children: [],
        }),
      );
    });
  ...
  const toDir = (item: File): void => {
    getFilesInDir(item, true);
  };
  ...
  const toLinkDir = (item: File): void => {
    const dir = item.isDir
      ? item.realFile
      : item.realFile.lastIndexOf('/') > -1
      ? item.realFile.substring(0, item.realFile.lastIndexOf('/'))
      : '/';
    ...
    getFilesInDir(
      new File({
        name: item.realFile,
        isDir: item.isDir,
        absnameKey: `${item.realFile}`.replaceAll('/', '_').replaceAll('.', '_'),
        absname: item.realFile,
        children: [],
      }),
      true,
    );
