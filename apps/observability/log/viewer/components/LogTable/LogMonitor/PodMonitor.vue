...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async podCPUUsage() {
        if (this.item) {
          let query = POD_CPU_USAGE_PROMQL.replaceAll('$1', this.item.stream?.pod).replaceAll(
            '$2',
      ...
      async podMemoryUsage() {
        if (this.item) {
          let query = POD_MEMORY_USAGE_PROMQL.replaceAll('$1', this.item.stream?.pod).replaceAll(
            '$2',
            this.item.stream?.namespace,
          );
          query = query.replaceAll('%', '');
      ...
      async podNetworkIn() {
        if (this.item) {
          let query = POD_NETWORK_IN_PROMQL.replaceAll('$1', this.item.stream?.pod).replaceAll(
            '$2',
            this.item.stream?.namespace,
          );
          query = query.replaceAll('%', '');
      ...
      async podNetworkOut() {
        if (this.item) {
          let query = POD_NETWORK_OUT_PROMQL.replaceAll('$1', this.item.stream?.pod).replaceAll(
            '$2',
            this.item.stream?.namespace,
          );
          query = query.replaceAll('%', '');
