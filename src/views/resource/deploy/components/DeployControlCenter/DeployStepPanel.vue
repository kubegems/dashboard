  ...
  >
    ...
    <template #action>
      <template
        v-if="
          status &&
            (status.strategy === 'Canary' || status.strategy === 'BlueGreen')
        "
      >
        <v-btn
          text
          small
          class="white--text"
          @click="deployControll('重启应用', 'restart')"
        >
          <v-icon
            left
            small
        ...
        <v-btn
          ...
          small
          class="white--text"
          @click="deployControll('重试发布', 'retry')"
        >
          <v-icon
            left
            small
        ...
        <v-btn
          ...
          small
          class="white--text"
          @click="deployControll('中止发布', 'pause')"
        >
          <v-icon
            left
            small
        ...
        <v-btn
          ...
          small
          class="white--text"
          @click="deployControll('继续发布', 'promote', { full: false })"
        >
          <v-icon
            left
            small
          ...
          <v-stepper-header>
            <template v-for="(step, index) in status ? status.steps : []">
              <v-stepper-step
                :key="index"
                :step="index + 1"
                :complete="index < getNowStep(status.step)"
              >
                <span
                  v-for="key in Object.keys(step)"
                  :key="key"
                >
                  {{ key }}
                  <span v-if="key === 'setWeight'">
                    : {{ step['setWeight'] }}
                  </span>
                  <span v-else-if="key === 'pause'">
                    {{
                      JSON.stringify(step['pause']) === '{}'
                        ? ''
                        : `: ${step['pause']['duration']}`
                    }}
                  </span>
