<template>
  <v-sheet class="pa-2">
    <BaseListItemForDetail title="存储类型">
      <template #content>
        ...
        <span
          v-if="
            pvc &&
              ...
              pvc.metadata.annotations[`pvc.${process.env.VUE_APP_DOMAIN}/in-use`] === 'true'
          "
        >
          <v-icon
            small
      ...
      <template #content>
        <span
          v-if="
            pvc &&
              pvc.metadata.annotations &&
              pvc.metadata.annotations[`pvc.${process.env.VUE_APP_DOMAIN}/allow-snapshot`] ===
              'true'
          "
        >
          <v-icon
