...
const select: { [key: string]: any } = {
  ...
  methods: {
    ...
    async m_select_namespaceSelectData(
      ...
      convertResponse2List(data as KubePaginationResponse<any[]>).forEach((ns) => {
    ...
    async m_select_storageClassSelectData(
      ...
      convertResponse2List(data as KubePaginationResponse<any[]>).forEach((sc) => {
    ...
    async m_select_workloadSelectData(Cluster: string, Namespace: string): Promise<void> {
      ...
      data = data
        .concat(convertResponse2List(ds as KubePaginationResponse<any[]>))
        .concat(convertResponse2List(deploy as KubePaginationResponse<any[]>))
        .concat(convertResponse2List(sts as KubePaginationResponse<any[]>));
    ...
    async m_select_secretSelectData(Cluster: string, Namespace: string, type: string = null): Promise<void> {
      ...
      let list = [];
      if (type) {
        list = convertResponse2List(data as KubePaginationResponse<any[]>).filter((d) => {
          return d.type === type;
        });
      }
      list.forEach((s) => {
    ...
    async m_select_serviceSelectData(Cluster: string, Namespace: string): Promise<void> {
      ...
      convertResponse2List(data as KubePaginationResponse<any[]>).forEach((s) => {
        const ports = [];
        const portNames = [];
        if (s.spec.ports) {
    ...
    async m_select_issuerSelectData(Cluster: string, Namespace: string): Promise<void> {
      ...
      convertResponse2List(data as KubePaginationResponse<any[]>).forEach((i) => {
        issuerSelect.push({
    ...
    async m_select_virtualSpaceSelectData(): Promise<void> {
      ...
      convertResponse2List(data as KubePaginationResponse<any[]>).forEach((r) => {
