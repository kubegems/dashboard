  ...
  <div>
    <v-card class="px-2">
      <BaseListItemForDetail title="标签" :mt="0">
        <template #content>
          <BaseCollapseChips v-if="item" :chips="item.service.labels || {}" single-line icon="mdi-label" />
        </template>
      ...
      <BaseListItemForDetail title="端口">
        <template #content>
          <v-flex class="text-body-2">
            <BaseCollapseChips v-if="item" :chips="services || []" single-line icon="mdi-directions-fork" />
          </v-flex>
        </template>
      </BaseListItemForDetail>
    </v-card>
