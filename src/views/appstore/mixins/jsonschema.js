  ...
  methods: {
    ...
    ConstructFormData(properties) {
      ...
      for (var item in properties) {
        ...
        if (
          Object.prototype.hasOwnProperty.call(properties[item], 'form') &&
          properties[item].form
        ) {
          if (properties[item].type === 'string') {
            propsformdata[item] =
              properties[item].default !== undefined
    ...
    ConstructPropertyRules(properties) {
      ...
      for (var item in properties) {
        ...
        if (
          ...
          if (properties[item].type === 'string') {
            if (
              properties[item].render &&
              properties[item].render === 'slider'
            ) {
              const min = properties[item].sliderMin
                ? properties[item].sliderMin
                : 1
              const max = properties[item].sliderMax
                ? properties[item].sliderMax
                : 100
              propsformdata[item].rules = [
                (v) => v >= min || '小于最小值',
                (v) => v <= max || '大于最大值',
              ]
            } else {
            ...
            propsformdata[item]['properties'] = this.ConstructPropertyRules(
              propsformdata[item].properties,
            )
          }
        } else {
          // 如果未开启表单, 则删除该object
