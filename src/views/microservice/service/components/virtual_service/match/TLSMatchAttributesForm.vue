          ...
          <v-form ref="form" v-model="valid" lazy-validation @submit.prevent>
            <v-sheet class="pt-2 px-2">
              ...
              <v-flex class="float-left ml-2 kubegems__long-width">
                <v-combobox
                  ...
                  hide-no-data
                  :items="[]"
                  :search-input.sync="sniHostsText"
                  :rules="objRules.sniHostsRule"
                  multiple
                  small-chips
                  label="sniHosts(回车)"
                  height="32"
            ...
            <v-sheet class="pt-2 px-2">
              ...
              <v-flex class="float-left ml-2 kubegems__form-width">
                <v-text-field v-model="obj.port" class="my-0" required label="port" type="number" />
              </v-flex>
              <v-flex class="float-left ml-2 kubegems__form-width">
                <v-text-field v-model="obj.sourceNamespace" class="my-0" required label="sourceNamespace" />
              </v-flex>
              <div class="kubegems__clear-float" />
            </v-sheet>
          ...
          <v-list-item-subtitle class="text-body-2 py-0">
            <v-list-item two-line class="float-left pa-0 kubegems__three-width">
              <v-list-item-content class="py-0">
                ...
                <v-list-item-subtitle class="text-body-2 py-1"> sniHosts </v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item two-line class="float-left pa-0 kubegems__three-width">
              <v-list-item-content class="py-0">
...
<script>
  ...
  export default {
    ...
    methods: {
      onSniHostsChange() {
        const sniHosts = this.sniHosts.filter((host) => {
          return host !== '' && typeof host === 'object';
        });
        this.sniHosts = sniHosts;
      },
      createSniHosts() {
        if (!this.sniHostsText) return;
        const index = this.sniHosts.length;
        this.sniHosts.push({
          text: this.sniHostsText,
          value: index,
        });
        this.sniHostsText = '';
      },
      ...
      addData() {
        if (this.$refs.form.validate(true)) {
          ...
          data.sniHosts = this.sniHosts.map((d) => {
            return d.text;
          });
      ...
      updateData(index) {
        ...
        this.obj.sniHosts.forEach((d, index) => {
          this.sniHosts.push({
            text: d,
            value: index,
          });
