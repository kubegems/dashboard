<template>
  <BaseDialog v-model="dialog" :width="500" title="快照恢复" icon="mdi-camera" @reset="reset">
    <template #content>
      ...
      <v-card-text class="px-2 pb-0">
        <v-form ref="form" v-model="valid" lazy-validation @submit.prevent>
          <v-sheet>
            ...
            <v-text-field v-model="obj.name" class="my-0" required label="存储卷名称" :rules="objRules.nameRules" />
          </v-sheet>
        </v-form>
      </v-card-text>
    </template>
    <template #action>
      <v-btn class="float-right" color="primary" text :loading="Circular" @click="restoreVolumeSnapshot"> 确定 </v-btn>
    </template>
  </BaseDialog>
</template>
