...
export default {
  ...
  methods: {
    async appRunningDetail() {
      const envid = this.$route.query.environmentid
    ...
    watchApp() {
      ...
      const watchAppList = []
      watchAppList.push(
        `${this.Environment().EnvironmentName}/${this.app.name}`,
      )
    ...
    removeApp() {
      this.$store.commit('SET_CONFIRM', {
        title:
          this.$route.query.kind === 'app'
            ? '删除平台应用'
            : '删除应用商店应用',
        content: {
          text:
            this.$route.query.kind === 'app'
              ? `删除平台应用 ${this.app.name}`
              : `删除应用商店应用 ${this.app.name}`,
          type: 'delete',
          name: this.app.name,
        },
        param: {},
