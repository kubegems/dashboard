...
<template>
  <v-card>
    ...
    <v-card-text :class="`clear-zoom-${Scale.toString().replaceAll('.', '-')}`">
      <v-row>
        <v-col cols="6">
          <BaseApexAreaChart id="cpu" label="container" :metrics="cpu" title="CPU使用量" type="cpu" />
        </v-col>
        <v-col cols="6">
          <BaseApexAreaChart id="memory" label="container" :metrics="memory" title="内存使用量" type="memory" />
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="6">
