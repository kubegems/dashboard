        ...
        <v-menu v-if="m_permisson_virtualSpaceAllow" left>
          ...
          <v-card>
            <v-card-text class="pa-2">
              <v-flex>
                <v-btn text color="primary" @click="addConfigMap">
                  <v-icon left>mdi-plus-box</v-icon>
                  创建配置
                </v-btn>
              </v-flex>
              <v-flex>
                <v-btn text color="error" @click="m_table_batchRemoveResource('配置', 'ConfigMap', configMapList)">
        ...
        <template #[`item.action`]="{ item }">
          ...
          <v-menu left :attach="`#r${item.metadata.resourceVersion}`">
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex>
                  <v-btn color="primary" text small @click.stop="updateConfigMap(item)"> 编辑 </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" text small @click.stop="removeConfigMap(item)"> 删除 </v-btn>
                </v-flex>
              </v-card-text>
            </v-card>
          </v-menu>
