<template>
  <BaseDialog v-model="dialog" :width="1000" title="创建项目" icon="mdi-cube-outline" @reset="reset">
    <template #content>
      <component :is="formComponent" :ref="formComponent" :step="step" />
    </template>
    <template #action>
      <v-btn v-if="step === 1" class="float-right mx-2" color="primary" text :loading="Circular" @click="addProject">
        确定
      </v-btn>
