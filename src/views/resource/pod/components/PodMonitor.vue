<template>
  <v-card>
    ...
    <v-card-text :class="`clear-zoom-${Scale.toString().replaceAll('.', '-')}`">
      <v-row>
        <v-col cols="6">
          <BaseApexAreaChart id="cpu" title="CPU使用量" :metrics="cpu" type="cpu" label="container" />
        </v-col>
        <v-col cols="6">
          <BaseApexAreaChart id="memory" title="内存使用量" :metrics="memory" type="memory" label="container" />
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="6">
