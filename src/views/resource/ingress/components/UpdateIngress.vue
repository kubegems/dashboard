<template>
  <BaseDialog v-model="dialog" :width="1000" title="更新路由" icon="mdi-network" @reset="reset">
    <template #content>
      <component :is="formComponent" :ref="formComponent" :item="item" :edit="true" title="Ingress" />
    </template>
    <template #action>
      <v-btn class="float-right" color="primary" text :loading="Circular" @click="updateIngress"> 确定 </v-btn>
    </template>
