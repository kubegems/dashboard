...
const apiVersion = apiResources['sidecar'] || 'networking.istio.io/v1beta1'

// 边车列表
export const getIstioSidecarList = (clusterName, namespace, query = {}) =>
  axios(
    `proxy/cluster/${clusterName}/${apiVersion}/namespaces/${namespace}/sidecar`,
    { params: query },
  )
// 边车详情
export const getIstioSidecarDetail = (clusterName, namespace, name) =>
  axios(
    `proxy/cluster/${clusterName}/${apiVersion}/namespaces/${namespace}/sidecar/${name}`,
  )
// 添加边车
