...
const integratedOpenTelemetry = () => {
  const resource = Resource.default().merge(
    new Resource({
      ...
      [SemanticResourceAttributes.SERVICE_VERSION]: import.meta.env.VUE_APP_RELEASE,
      'user.name': store.state?.User?.Username,
    }),
  );
  ...
  const exporter = new OTLPTraceExporter({
    url: import.meta.env.VUE_APP_OTEL_COLLECTOR,
    headers: {},
    concurrencyLimit: 10,
  });
...
if (import.meta.env.VUE_APP_OTEL === 'true') {
