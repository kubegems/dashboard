    ...
    <v-card>
        ...
        <v-menu v-if="m_permisson_resourceAllow" left>
          ...
          <v-card>
            <v-card-text class="pa-2">
              <v-flex>
                <v-btn text color="primary" @click="addService">
                  <v-icon left>mdi-plus-box</v-icon>
                  创建服务
                </v-btn>
              </v-flex>
              <v-flex>
                <v-btn text color="error" @click="m_table_batchRemoveResource('服务', 'Service', serviceList)">
      ...
      >
        ...
        <template #[`item.ports`]="{ item }">
          <BaseCollapseChips :chips="item.services || []" single-line icon="mdi-directions-fork" />
        </template>
        ...
        <template #[`item.action`]="{ item }">
          ...
          <v-menu left :attach="`#r${item.metadata.resourceVersion}`">
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex>
                  <v-btn color="primary" text small @click="updateService(item)"> 编辑 </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" text small @click="removeService(item)"> 删除 </v-btn>
                </v-flex>
              </v-card-text>
            </v-card>
          </v-menu>
