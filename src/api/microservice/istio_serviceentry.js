...
const apiVersion = apiResources['serviceentry'] || 'networking.istio.io/v1beta1'

// 服务入口列表
export const getIstioServiceEntryList = (clusterName, namespace, query = {}) =>
  axios(
    `proxy/cluster/${clusterName}/${apiVersion}/namespaces/${namespace}/serviceentry`,
    { params: query },
  )
// 服务入口详情
export const getIstioServiceEntryDetail = (clusterName, namespace, name) =>
  axios(
    `proxy/cluster/${clusterName}/${apiVersion}/namespaces/${namespace}/serviceentry/${name}`,
  )
// 添加服务入口
