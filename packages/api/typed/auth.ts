...
export class Auth implements AccessTokenOperator {
  ...
  public async getLoginRequestUrl(params: KubeRequest = {}): Promise<string> {
    const data = await axios(`/oauth/addr`, { params: params });
    return data as any;
  }
  ...
  public async login(body: KubeRequest, params: KubeRequest = {}): Promise<{ token: string }> {
    const data: { [key: string]: any } = await axios.post(`login`, body, { params: params });
