<template>
  <BaseDialog v-model="dialog" :width="1000" title="创建存储卷" icon="mdi-database-plus" @reset="reset">
    <template #content>
      <component :is="formComponent" :ref="formComponent" title="PersistentVolumeClaim" />
    </template>
    <template #action>
      <v-btn class="float-right" color="primary" text :loading="Circular" @click="addPersistentVolumeClaim">
        确定
      </v-btn>
    </template>
