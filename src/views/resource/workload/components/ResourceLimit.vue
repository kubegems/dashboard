<template>
  ...
  >
    <template #content>
      <v-card
        v-if="adviseItem && item"
        flat
      >
        <v-card-text
          v-for="obj in item.spec.template.spec.containers"
          :key="obj.name"
          class="pa-0 mb-8"
        >
          <div v-if="hasAdvise(obj.name)">
            ...
            <v-card-text class="pa-0">
              <p class="text-subtitle-2 px-2 primary--text mt-0">
                CPU预留/限制
              </p>
              <v-card-text
              ...
              >
                ...
                >
                  <template #tooltip="props">
                    <div class="tooltip">
                      <span>{{
                        getUnitValue(props.value, sliderItems[obj.name].cpu)
                      }}</span>
                    </div>
                  </template>
                </vue-slider>
            ...
            >
              ...
              >
                <template #tooltip="props">
                  <div class="tooltip">
                    <span>{{
                      getUnitValue(props.value, sliderItems[obj.name].memory)
                    }}</span>
                  </div>
                </template>
              </vue-slider>
  ...
  computed: {
    ...
    isArgoApp() {
      return (
        ...
        this.item.metadata.annotations['gems.cloudminds.com/ref']
      )
    },
  },
  methods: {
    ...
    getAdviseShow(c) {
      ...
      if (containerAdvise) {
        return (
          containerAdvise.CPUStatus.Suggestion +
          containerAdvise.MemoryStatus.Suggestion
        )
      }
    },
      ...
      this.workload.spec.template.spec.containers.forEach((c) => {
        if (this.sliderItems[c.name]) {
          c.resources.requests.cpu =
            this.sliderItems[c.name].cpu.mapValue[
              this.sliderItems[c.name].cpu.value[0]
            ]
          c.resources.limits.cpu =
            this.sliderItems[c.name].cpu.mapValue[
              this.sliderItems[c.name].cpu.value[1]
            ]
          c.resources.requests.memory =
            this.sliderItems[c.name].memory.mapValue[
              this.sliderItems[c.name].memory.value[0]
            ]
          c.resources.limits.memory =
            this.sliderItems[c.name].memory.mapValue[
              this.sliderItems[c.name].memory.value[1]
            ]
    ...
    async workloadDetail() {
      ...
      this.workload.spec.template.spec.containers.forEach((c) => {
        ...
        // 当前建议值
        const adviseCpuLimits = this.getValueNoUnit(
          c.resources.limits.cpu,
          'CPU',
        )
        ...
        switch (this.adviseItem.Conditions[c.name]['CPUStatus'].Action) {
          ...
          case 'scaleDown':
            ...
            break
          default:
            cpuMaxLimitWithUnit = resourcesBackup.limits
              ? resourcesBackup.limits.cpu
              : 0
        }
        // 获取最靠近的值, 先转为核
        const cpuMaxLimitClosest = this.getClosest(
          this.cpuRange,
          this.getValueNoUnit(cpuMaxLimitWithUnit, 'CPU'),
        )
        ...
        // 寻找index
        const cpuSuggestMaxLimitClose = this.getClosest(
          cpuMapValueNoUnit,
          cpuSuggestMaxLimit,
        )
        const cpuSuggestMinLimitClose = this.getClosest(
          cpuMapValueNoUnit,
          cpuSuggestMinLimit,
        )

        const cpuSuggestLimitClose = this.getClosest(
          cpuMapValueNoUnit,
          adviseCpuLimits,
        )

        const cpuSuggestMaxLimitIndex = cpuMapValueNoUnit.indexOf(
          cpuSuggestMaxLimitClose,
        )
        const cpuSuggestMinLimitIndex = cpuMapValueNoUnit.indexOf(
          cpuSuggestMinLimitClose,
        )
        const cpuSuggestLimitIndex =
          cpuMapValueNoUnit.indexOf(cpuSuggestLimitClose)
        // 标记mark
        this.$set(
          sliderItem.cpu.marks,
          cpuSuggestMaxLimitIndex,
          sliderItem.cpu.mapValue[cpuSuggestMaxLimitIndex],
        )
        this.$set(
          sliderItem.cpu.marks,
          cpuSuggestMinLimitIndex,
          sliderItem.cpu.mapValue[cpuSuggestMinLimitIndex],
        )

        // 设置双滑块左右值
        this.setValue(
          sliderItem.cpu,
          this.getBeautifyCpuUnit(
            this.getClosest(cpuMapValueNoUnit, cpuRequests),
          ),
          0,
        )
        this.setValue(
          sliderItem.cpu,
          sliderItem.cpu.mapValue[cpuSuggestLimitIndex],
          1,
        )
        // 内存
        const memoryRequests = this.getValueNoUnit(
          c.resources.requests.memory,
          'Memory',
        )

        const adviseMemoryLimits = this.getValueNoUnit(
          c.resources.limits.memory,
          'Memory',
        )
        ...
        switch (this.adviseItem.Conditions[c.name]['MemoryStatus'].Action) {
          case 'scaleUp':
            memoryMaxLimitWithUnit = this.getAdviseMaxValue(
              c.name,
              'MemoryStatus',
            )
            break
        ...
        // 寻找index
        const memorySuggestMaxLimitClose = this.getClosest(
          memoryMapValueNoUnit,
          memorySuggestMaxLimit,
        )
        const memorySuggestMinLimitClose = this.getClosest(
          memoryMapValueNoUnit,
          memorySuggestMinLimit,
        )
        const memorySuggestLimitClose = this.getClosest(
          memoryMapValueNoUnit,
          adviseMemoryLimits,
        )
        const memorySuggestMaxLimitIndex = memoryMapValueNoUnit.indexOf(
          memorySuggestMaxLimitClose,
        )
        const memorySuggestMinLimitIndex = memoryMapValueNoUnit.indexOf(
          memorySuggestMinLimitClose,
        )
        const memorySuggestLimitIndex = memoryMapValueNoUnit.indexOf(
          memorySuggestLimitClose,
        )
        // 标记mark
        this.$set(
        ...
        this.setValue(
          sliderItem.memory,
          this.getBeautifyStorageUnit(
            this.getClosest(memoryMapValueNoUnit, memoryRequests),
          ),
          0,
        )
        this.setValue(
          sliderItem.memory,
          sliderItem.memory.mapValue[memorySuggestLimitIndex],
          1,
        )
    ...
    getValueNoUnit(valueWithUnit, type = 'CPU') {
      if (type === 'CPU') {
        return sizeOfCpu(
          !isNaN(valueWithUnit) ? `${valueWithUnit}core` : valueWithUnit,
          'core',
        )
      } else if (type === 'Memory') {
        ...
        return valueWithUnit
          ? sizeOfStorage(valueWithUnit.replace(regExp, '$1' + 'i'), 'Gi')
          : 0
      }
    },
    ...
    getBeautifyCpuUnit(inputValue) {
      ...
      const result = beautifyCpuUnit(
        inputValue * 1000 * 1000 * 1000,
        1,
      ).replace(regExp, '')
    ...
    getBeautifyStorageUnit(inputValue) {
      ...
      const result = beautifyStorageUnit(
        inputValue * 1024 * 1024 * 1024,
        1,
      ).replace(regExp, '')
      return result === '' ? 0 : result
    },
    ...
    generateMapValue(sliderObj, type = 'CPU') {
      ...
      sliderObj.mapValue = Array.from(
        { length: sliderObj.max + 1 },
        (item, index) => {
          return (sum +=
            perNum *
            Math.pow(
        ...
        return type === 'CPU'
          ? this.getBeautifyCpuUnit(item - perNum)
          : this.getBeautifyStorageUnit(item - perNum)
      })
    },
