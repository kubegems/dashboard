...
<template>
  <v-form ref="form" v-model="valid" class="mt-2" lazy-validation @submit.prevent>
    <v-row>
      ...
      <v-col cols="6">
        <v-text-field
          v-model="obj.channelConfig.signName"
          class="my-0"
          label="SignName"
          required
          :rules="objRules.signNameRule"
        />
      </v-col>
      <v-col cols="6">
        <v-text-field
          v-model="obj.channelConfig.templateCode"
          class="my-0"
          label="TemplateCode"
          required
          :rules="objRules.templateCodeRule"
        />
      </v-col>
      <v-col cols="12">
        <v-autocomplete
          ...
          :rules="objRules.phoneNumbersRule"
    ...
    data() {
      return {
        ...
        objRules: {
          ...
          signNameRule: [required],
          templateCodeRule: [required],
          phoneNumbersRule: [required],
        },
        phoneToItems: [],
        phoneTo: [],
        phoneText: '',
      ...
      data: {
        handler(newValue) {
          if (newValue) {
            ...
            const phones = this.obj.channelConfig.phoneNumbers.split(',').filter((t) => {
              return Boolean(t);
            });
            this.phoneTo = phones;
