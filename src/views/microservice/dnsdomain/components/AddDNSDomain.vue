<template>
  <BaseDialog v-model="dialog" :width="500" title="创建DNS" icon="mdi-dns" @reset="reset">
    <template #content>
      <component :is="formComponent" :ref="formComponent" />
    </template>
    <template #action>
      <v-btn class="float-right" color="primary" text :loading="Circular" @click="addDNSDomain"> 确定 </v-btn>
    </template>
  </BaseDialog>
</template>
