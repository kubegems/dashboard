    ...
    <v-card>
        ...
        <v-menu v-if="m_permisson_resourceAllow" left>
          ...
          <v-card>
            <v-card-text class="pa-2">
              <v-flex>
                <v-btn text color="primary" @click="addCronJob">
                  <v-icon left>mdi-playlist-plus</v-icon>
                  创建定时任务
                </v-btn>
              </v-flex>
              <v-flex>
                <v-btn text color="error" @click="m_table_batchRemoveResource('定时任务', 'CronJob', cronJobList)">
      ...
      >
        ...
        <template #[`item.status`]="{ item }">
          ...
          <span v-else>
            <v-avatar style="background-color: #607d8b" class="mr-2" size="10">
              <span class="white--text text-h5" />
            </v-avatar>
            Suspend
          </span>
        ...
        <template #[`item.action`]="{ item }">
          ...
          <v-menu left :attach="`#r${item.metadata.resourceVersion}`">
            ...
            <v-card>
              <v-card-text class="pa-2">
                <v-flex>
                  <v-btn color="primary" text small @click="updateCronJob(item)"> 编辑 </v-btn>
                </v-flex>
                <v-flex>
                  <v-btn color="error" text small @click="removeCronJob(item)"> 删除 </v-btn>
                </v-flex>
              </v-card-text>
            </v-card>
          </v-menu>
