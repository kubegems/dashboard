      ...
      >
        <v-sheet class="pa-2">
          <v-flex class="text-subtitle-1 mb-2">
            ...
            当前
            <span class="text-subtitle-2 primary--text">
              {{ statistics ? statistics.Cpu : 0 }} core
            </span>
          </v-flex>
          <v-text-field
      ...
      >
        <v-sheet class="pa-2">
          <v-flex class="text-subtitle-1 mb-2">
            ...
            当前
            <span class="text-subtitle-2 primary--text">
              {{ statistics ? statistics.Memory : 0 }} Gi
            </span>
          </v-flex>
          <v-text-field
      ...
      >
        <v-sheet class="pa-2">
          <v-flex class="text-subtitle-1 mb-2">
            可用存储
            <span class="text-subtitle-2 primary--text">
              {{ quota ? quota.AllocatedStorage.toFixed(1) : 0 }} Gi
            </span>
            当前
            <span class="text-subtitle-2 primary--text">
              {{ statistics ? statistics.Storage : 0 }} Gi
            </span>
          </v-flex>
          <v-text-field
    ...
    objRules() {
      return {
        cpuRules: [
          required,
          (v) =>
            !!new RegExp('^([0-9][0-9]*)+(.[0-9]{1,2})?$').test(v) ||
            '格式错误(示例:整数)',
          (v) =>
