    ...
    <v-expand-transition>
      ...
      >
        <v-sheet class="pt-4 px-2">
          <v-flex
            class="float-left text-body-2 py-2 primary--text font-weight-medium kubegems__min-width"
          >
            <span>容器CPU</span>
          </v-flex>
      ...
      return {
        container: {
          ...
          cpuMaxRule: [
            ...
            (v) =>
              sizeOfCpu(v) >=
                sizeOfCpu(this.obj.LimitRange.Container.defaultRequest.cpu) ||
              'Limits小于Requests',
          ],
          memoryDefaultRule: [
            (v) =>
              !!new RegExp('(^\\d+[M|G]i$)|(^0$)').test(v) ||
              '格式错误(示例:1Mi,1Gi)',
          ],
          ...
          memoryMaxRule: [
            ...
            (v) =>
              sizeOfStorage(v) >=
                sizeOfStorage(
                  this.obj.LimitRange.Container.defaultRequest.memory,
                ) || 'Limits小于Requests',
          ],
        },
        pod: {
          cpuMaxRule: [
            (v) => !!new RegExp('^\\d+[m]?$').test(v) || '格式错误(示例:1m,1)',
            (v) =>
              sizeOfCpu(v) >= sizeOfCpu(this.obj.LimitRange.Pod.min.cpu) ||
              'Limits小于Requests',
          ],
          memoryMaxRule: [
            (v) =>
            ...
            (v) =>
              sizeOfStorage(v) >=
                sizeOfStorage(this.obj.LimitRange.Pod.min.memory) ||
              'Limits小于Requests',
          ],
        },
        persistentVolumeClaim: {
          storageMaxRule: [
            ...
            (v) =>
              sizeOfStorage(v) >=
                sizeOfStorage(
                  this.obj.LimitRange.PersistentVolumeClaim.min.storage,
                ) || 'Limits小于Requests',
          ],
        },
      }
