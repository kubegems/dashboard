<template>
  ...
  >
    ...
    <v-sheet
      v-if="containerCopy && (containerCopy.command || containerCopy.args)"
      class="grey lighten-4 rounded ma-2"
    >
      <v-list-item two-line>
        <v-list-item-content class="py-2">
          <v-list-item-subtitle
            v-for="(command, index) in containerCopy.command
              ? containerCopy.command
              : []"
            :key="`${index}command`"
          ...
          <v-list-item-subtitle
            v-for="(args, index) in containerCopy.args
              ? containerCopy.args
              : []"
            :key="`${index}args`"
            class="text-body-2 py-0"
