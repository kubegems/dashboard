    ...
    >
      ...
      <template #[`item.status`]="{ item }">
        ...
        <span v-else-if="item.state.terminated">
          Terminated({{ item.state.terminated.reason }})
        </span>
        <span v-else-if="item.state.waiting">
          Waiting({{ item.state.waiting.reason }})
        </span>
      </template>
            ...
            <v-card-text class="pa-2 text-center">
              <v-flex v-if="resourceAllow && item.state.running !== undefined">
                <v-btn
                  ...
                  text
                  small
                  @click="containerShell(item.name)"
                >
                  终端
                </v-btn>
              </v-flex>
              <v-flex v-if="resourceAllow && item.state.running !== undefined">
                <v-btn
                  ...
                  text
                  small
                  @click="containerDebug(item.name)"
                >
                  Debug
                </v-btn>
              </v-flex>
      ...
      if (this.item && this.item.status && this.item.status.containerStatuses) {
        ...
        if (data) {
          data.forEach((d) => {
            ...
            MemoryLatest[d.metric.container] = latest
              ? beautifyStorageUnit(latest)
              : 0
