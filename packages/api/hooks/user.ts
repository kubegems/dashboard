...
import { Tenant } from '../typed/tenant';
...
export const useUserTenantList = async (user: User): Promise<Tenant[]> => {
  const _data: KubePaginationResponse<Tenant[]> = await user.getTenantList({
    page: 1,
    size: 1000,
    noprocessing: true,
