...
import { validateJWT } from 'src/utils/helpers';

axios.defaults.baseURL = `${window.location.origin}/api/v1`;
axios.defaults.timeout = 1000 * parseInt(import.meta.env.VUE_APP_API_TIMEOUT);

axios.interceptors.request.use((config: AxiosRequestConfig<any>): AxiosRequestConfig<any> | Promise<any> => {
  if (
...
axios.interceptors.response.use(
  (response: AxiosResponse<any, any>): Promise<void> => {
