<template>
  <BaseDialog v-model="dialog" :width="1000" title="更新采集器" icon="mdi-arrange-send-backward" @reset="reset">
    <template #content>
      <component :is="formComponent" :ref="formComponent" :item="item" :edit="true" title="Flow/ClusterFlow" />
    </template>
    <template #action>
      <v-btn class="float-right" color="primary" text :loading="Circular" @click="updateFlow"> 确定 </v-btn>
    </template>
