...
<template>
  <v-flex>
    <v-sheet v-for="(t, index) in tcp" :key="index" class="grey lighten-4 rounded mb-3">
...
<script>
  export default {
    ...
    methods: {
      updateTcp(index) {
        this.$emit('updateTcp', index);
      },
      removeTcp(index) {
        this.$emit('removeTcp', index);
      },
      expandCard() {
        this.$emit('expandCard', 'tcpForm');
      },
    },
  };
</script>
