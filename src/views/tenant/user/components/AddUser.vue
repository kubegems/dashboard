<template>
  <BaseDialog v-model="dialog" :width="500" title="创建用户" icon="mdi-account" @reset="reset">
    <template #content>
      <BaseSubTitle title="用户定义" />
      <v-card-text class="pa-2 mt-2">
        <v-form ref="form" v-model="valid" lazy-validation @submit.prevent>
          <v-sheet>
            <v-text-field v-model="obj.Username" class="my-0" :rules="objRules.usernameRules" required label="用户名" />
            <v-text-field v-model="obj.Password" class="my-0" :rules="objRules.passwordRules" required label="密码">
              <template #append>
            ...
            <v-text-field v-model="obj.Email" class="my-0" :rules="objRules.emailRules" required label="邮箱" />
          </v-sheet>
        </v-form>
      </v-card-text>
    </template>
    <template #action>
      <v-btn class="float-right" color="primary" text :loading="Circular" @click="addUser"> 确定 </v-btn>
    </template>
  </BaseDialog>
</template>
